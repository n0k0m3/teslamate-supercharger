...
- **`tesla_api.py`** — Acquires a Fleet API token via `client_credentials` grant from `fleet-auth.prd.vn.cloud.tesla.com` using `TESLA_CLIENT_ID`/`TESLA_CLIENT_SECRET`. Token lifetime is 8h; re-acquired on 401. `fetch_charging_history()` fetches one page; `fetch_all_charging_history()` paginates all results.
