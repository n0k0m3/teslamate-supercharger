...
- **`tesla_api.py`** — Calls `https://www.tesla.com/teslaaccount/charging/api/history`. Handles 401 by refreshing via `https://auth.tesla.com/oauth2/v3/token` and retrying once. The 404 warning is intentional — the endpoint may change; check for a GraphQL alternative if it consistently 404s.
